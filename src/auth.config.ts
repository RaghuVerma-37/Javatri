 ...
 * `node:url` — cannot load. So the configuration is split: everything the middleware needs to
 ...
 *
 * Verifying the session is pure JWT work, so the middleware loses nothing by not having the
 * database.
 */
export const authConfig = {
  ...
  trustHost: true,
  // Filled in by auth.ts. The middleware never signs anyone in, only checks who they are.
  providers: [],
  callbacks: {
    jwt({ token, user }) {
